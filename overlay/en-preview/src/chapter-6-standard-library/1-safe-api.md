...
Therefore, before calling `safe_free` to release memory, you need to explicitly cast the `_Owned` pointer to the `void * _Owned` type. For the specific casting rules, see [Ownership State Transfer Rules - Type Casting](../chapter-3-memory-safety/1-ownership.md#type-casts).
Some specific examples of usage are given below.
