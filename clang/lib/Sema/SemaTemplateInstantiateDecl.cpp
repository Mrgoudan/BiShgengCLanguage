...
Decl *TemplateDeclInstantiator::VisitBSCMethodDecl(
  ...
  Method = BSCMethodDecl::Create(SemaRef.Context, Record, StartLoc, NameInfo, T,
                                 TInfo, SC, D->isInlineSpecified(), false,// isInline?
  ...
  if (DependentFunctionTemplateSpecializationInfo *Info =
    ...
    TemplateArgumentListInfo ExplicitArgs(Info->getLAngleLoc(),
                                          Info->getRAngleLoc());
    if (SemaRef.SubstTemplateArguments(Info->arguments(), TemplateArgs, ExplicitArgs))
      return nullptr;

    // Map the candidate templates to their instantiations.
    for (unsigned I = 0, E = Info->getNumTemplates(); I != E; ++I) {
      Decl *Temp = SemaRef.FindInstantiatedDecl(
          D->getLocation(), Info->getTemplate(I), TemplateArgs);
      if (!Temp)
        return nullptr;
