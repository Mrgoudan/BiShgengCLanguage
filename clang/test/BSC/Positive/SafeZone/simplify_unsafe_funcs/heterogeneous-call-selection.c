...
// Test 1: Basic selection - owned argument selects _Safe version
...
void test1(void) {
    ...
    int* owned result = process(owned_p);    // Calls _Safe version
...
void test5(void) {
    ...
    int* owned result2 = get_int(owned_p); // Calls _Safe version
...
void test7_safe(void) {
    ...
    _Safe {
        int* owned result = process(p);  // Must call _Safe version (only _Safe allowed in _Safe context)
        safe_free((void* owned)result);
    }
}
...
void test8_unsafe(void) {
    ...
    _Unsafe {
        int* owned result1 = process(owned_p);  // Prefers _Safe version (constraints match)
