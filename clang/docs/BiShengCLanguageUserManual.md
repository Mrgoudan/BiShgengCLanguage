...
5. trait 指针类型的变量，允许用 `NULL` 初始化（待实现）

```c
trait Print* p = NULL;
```

6. 允许指向 trait 的多级指针，但这种类型不能直接调用成员函数。不允许结构体的二级指针直接隐式转换成trait的二级指针（待实现）
...
struct S s;
q = &&s // error: 不允许结构体的二级指针直接隐式转换成trait的二级指针
```
...
2. 实现了 trait 的类型指针可以隐式/强制转换为 trait 指针类型

```c
int a = 0;                 // 假设 int 类型实现了 trait T
trait T* t = &a;           // 隐式转换
trait T* t = (trait T*)&a; // 强制转换
```

3. 不支持 trait 指针类型 `down_cast` 到具体类型，但支持转为 `void *`类型 (待实现)
