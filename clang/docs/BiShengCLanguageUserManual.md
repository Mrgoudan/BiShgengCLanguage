...
1. 引入新关键字`borrow`，用`borrow`来修饰类型 T，表示 T 的借用类型
...
##### 3.2 可变借用同时只能存在一个
毕昇 C 规定，**同一时刻，对于同一个对象，只能拥有要么一个可变借用, 要么任意多个不可变借用**，这种限制的好处是可以在编译期避免数据竞争。
数据竞争可由以下行为造成：
1. 两个或更多的指针同时访问同一数据
2. 至少有一个指针被用来写入数据
数据竞争会导致未定义行为，这种行为很可能超出我们的预期，难以在运行时追踪，并且难以诊断和修复。而毕昇 C 避免了这种情况的发生，它可以在编译期识别出存在数据竞争的代码，例如：
```C
void test1() {
    int local = 5;
...
```
由于不可变借用不会导致数据竞争，因此同一时刻可以拥有任意多个不可变借用，例如：
```C
void test() {
    int local = 5;
...
这条规则同样是为了避免数据竞争，因为使用可变借用，可能会导致被借用对象的内存状态发生改变，如果后续通过其它借用变量访问这块被修改的内存，可能会导致未定义行为。
例如：
```C
struct A {
    int *p;
