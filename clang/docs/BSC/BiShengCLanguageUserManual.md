...
1. 泛型成员函数的返回类型可以是泛型 struct 或泛型 union类型。
...
1. 函数指针可以使用 constexpr 修饰，函数指针可以指向 constexpr 函数
2. constexpr 可以修饰指针变量，但其只能指向全局变量或静态变量
...
1. 在非“常量计算”的上下文中，constexpr 修饰的函数可以当作普通函数使用，实参不需要是常量，返回值也不需要是常量。在“常量计算”的上下文中，实参和返回值都要求是常量表达式，否则会报错
...
2. constexpr 不允许支持变长参数
3. 函数的形参不能用 constexpr 修饰
...
1. trait 内要求函数首个入参且只有首个入参类型为 `This` 指针，命名为 `this`; `This` 指代实现了 trait 的具体类型
...
1. 如果 `<type>` 实现了 `<trait>`，那么指向这个 `<type>` 的指针可以被转换为 `<trait>` 类型的变量
...
1. trait 指针类型的变量可以通过 `->` 方式调用该 trait 中的成员方法
...
1. trait 指针类型的变量，允许用 `NULL` 赋值

```c
trait Print* p = NULL;
```

1. 允许指向 trait 的多级指针，但这种类型不能直接调用成员函数。不允许结构体的二级指针直接隐式转换成 trait 的二级指针
...
1. trait 指针类型变量可以用 `const` / `volatile` 修饰
...
1. 支持 trait 指针变量和 `NULL` 做比较（这里的比较仅包含 `==` 和`!=`，下同）
...
1. 支持 trait 指针变量和非 trait 指针变量比较
...
1. trait 类型支持强制转换为 `void *`类型，但`void *` 指针无法转换为 trait 指针类型
...
1. 泛型 trait 内要求函数首个入参且只有首个入参类型为 `This` 指针，命名为 `this`; `This` 指代实现了 trait 的具体类型
...
1. 泛型 trait 内可以没有方法签名

```c
trait F<T> {}; // ok
```

1. 不允许给泛型 trait 类型扩展成员函数
...
1. 在 impl 语句之前，`<type>` 必须已经声明了 `<trait>` 中所有成员函数
...
1. 支持通过 `impl` 关键字对已有 `struct`/`union`/`enum` 类型和内置类型实现 `trait<int>`/`trait<float>`，但`struct`/`union` 类型类型不能是泛型的
...
1. 泛型 trait 实例化后的指针类型变量可以通过 `->` 方式调用该泛型 trait 中的成员函数
...
1. 泛型 trait 实例化后的指针类型变量，允许用 `NULL` 赋值

```c
trait F<int>* p = NULL;
```

1. 泛型 trait 实例化后的指针类型变量，不可以解引用
...
1. 允许指向泛型 trait 的多级指针，但这种类型不能直接调用成员函数。不允许结构体的二级指针直接隐式转换成泛型 trait 的二级指针
...
1. 泛型 trait 实例化后的指针类型变量可以用 `const` / `volatile` 修饰
...
1. 支持泛型 trait 实例化后的指针类型作为函数参数及返回值类型
...
1. 支持泛型 trait 实例化后的指针变量和 `NULL` 做比较（这里的比较仅包含 `==` 和`!=`，下同）
...
1. 支持泛型 trait 实例化后的指针变量和非 trait 指针变量比较
...
1. 支持泛型 trait 指针类型强制转为非 trait 指针类型，但不支持隐式转换
...
1. 泛型 trait 类型支持强制转换为 `void *`类型，但`void *` 指针无法转换为泛型 trait 指针类型
...
1. `async` 函数中可以出现多个不同或相同 `await` 表达式
...
1. `await` 表达式不能出现在 if/while/for/do-while 等判断条件中
...
1. `await` 表达式不能和“有副作用”的表达式（例如函数）并存
...
1. `await` 表达式不能出现在复合表达式中，例如：表达式中含有 +、-、*、/、%、&、|、>>、<< 等
...
2. `safe`修饰的函数，函数参数列表不可以包含变长参数，除非该函数使用了`__attribute__((format(...)))`属性。例如:
...
1. 安全区内不允许访问`union`的成员（读或写），不允许裸指针通过`->`访问成员。
...
void test(void) {
  ...
  safe {
    ...
    union un b = {10};
    // error: 安全区不允许通过"."访问 union 成员（读操作）
    int c = b.age;
    // error: 安全区不允许通过"."访问 union 成员（写操作）
    b.age = 20;
...
1. 基本类型一致时，允许`owned`指针类型与非`owned`指针类型之间的显式强制类型转换，且这种转换只能在非安全区进行；
...
1. `owned`可以修饰 trait 类型，即`trait T* owned`，也表示该变量拥有其内部存储的数据的所有权。
...
1. 不允许在 `T *owned` 和 `T *borrow` 指针之间使用 C 风格强制类型转换
...
5. 允许对借用类型使用 `sizeof`、`alignof`操作符，并且有：
...
```C
// 返回值类型为 Nullable
safe T *borrow _Nullable nullable<T>(T *borrow p) { return p; }
// 返回值类型为 Nonnull
safe T *borrow nonnull<T>(T *borrow p) { return p; }
...
safe void test(void) {
  int local = 10;
  // error: 安全区内禁止用 nullable 值给 nonnull 指针赋值
  int *borrow p1 = nullptr;
  // error: 安全区内禁止用 nullable 值给 nonnull 指针赋值
  int *borrow p2 = nullable(&mut local);
  int *borrow p3 = nonnull(&mut local); // ok

  int *borrow _Nullable p4 = nullable(&mut local);
  // error: 安全区内禁止用 nullable 值给 nonnull 指针赋值
  int *borrow p5 = p4;

  int *borrow _Nullable p6 = nonnull(&mut local);
  int *borrow p7 = p6;
}
...
1. 函数调用时，如果形参为 Nonnull 类型，那么不能使用用可空表达式作为实参：
...
safe void take_nonnull(int *borrow p) {}

safe void test(void) {
  int local = 10;
  int *borrow _Nullable p = nullable(&mut local);
  // error: 要求 nonnull 指针，但实参是 nullable
  take_nonnull(p);
}
