...
```
另外，表达式`e`如果是指针的解引用表达式，`&_Mut *p`和`&_Const *p`分别可以看作对地址`p`中存放的值，也就是`*p`，取可变借用和不可变借用，**这一操作不为`*p`产生临时变量**。其中，`p`可以是裸指针、`owned`指针和其它借用指针。例如：
```C
#include "bishengc_safety.hbs" // BiShengC 语言提供的头文件，用于安全地进行内存分配及释放
...
| &_Const *e | *e 进入 “只读” 状态，不允许修改 *e 及其直接或间接成员，或者对其它成员做可变借用，如果 e 是 owned 指针类型，则 e 也进入只读状态 |
...
| &_Mut *e | *e 被冻结，不允许读写 *e 以及它的成员，如果 e 是 owned 指针类型，则也不允许读写 e |
